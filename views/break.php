...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($faceImage)) {
        ...
        // ✅ Facial Break API
        $url = "http://localhost:8080/api/user/facial/break";
        ...
        if ($result === FALSE) {
            echo "<script>alert('Unable to connect to attendance server. Please try again.');</script>";
        } else {
            $response = json_decode($result, true);
            $message  = $response["message"] ?? "Break process failed.";

            // ✅ BREAK IN
            if ($message === "Break-in recorded successfully") {
                echo "<script>
                    alert('Break-in successful. Enjoy your break ☕');
                </script>";

            // ✅ BREAK OUT
            } elseif ($message === "Break-out recorded successfully") {
                echo "<script>
                    alert('Break-out successful. Welcome back 👋');
                </script>";
...
<!DOCTYPE html>
        ...
        <head>
          ...
          <title>Record Break - AgriTime Payroll Attendance System</title>
          <!-- Face API scripts -->
         ...
         <body class="login-page">
          ...
          <div class="left">
            <h2 class="login-title">Record Employee Break</h2>
            <p class="login-subtext">You are leaving the system
                <br>Scan here to record break time
                </p>
      ...
      onDetect: (label, detection) => {
        ...
        if (detectionCounts[label] >= 100) {
          ...
          const intervalId = setInterval(() => {
            countdown--;
            if (countdown > 0) {
              messageEl.textContent = `Welcome back, ${label}! break has been enabled ...`;
            } else {
